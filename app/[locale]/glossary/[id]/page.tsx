    ...
    <div className="flex min-h-screen flex-col justify-between">
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-12 md:py-16">
        <div>
          ...
          <p className="mt-3 text-lg font-medium text-muted-foreground leading-relaxed">{entity.definition}</p>
            ...
            <ul className="flex flex-wrap gap-2">
              {related.map((r) => (
                <li key={r.id}>
                  <Link
                    href={`/glossary/${r.id}`}
                    className="inline-block rounded-full border px-3 py-1 text-xs font-medium text-muted-foreground hover:text-foreground hover:border-emerald-500 transition-colors"
                  >
                    {r.name}
                  </Link>
                </li>
