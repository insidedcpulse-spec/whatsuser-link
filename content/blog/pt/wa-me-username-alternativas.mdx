...
O gerador deste site cria já o teu link no formato `wa.me/u/username` (uma aposta razoável, alinhada com convenções que a WhatsApp já usa noutros contextos), mas deixa sempre claro — como este artigo — que esse link ainda não abre conversa sozinho. Se geres um projeto técnico ou pretendes criar links e validar contactos de forma programática, podes consultar a nossa [API REST pública](/pt/glossary/api) gratuita. Assim que a WhatsApp publicar o formato oficial, tanto a plataforma como a [API](/pt/glossary/api) serão atualizadas automaticamente, sem qualquer ação da tua parte.
