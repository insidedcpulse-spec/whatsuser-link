...
5. **Consider enabling a [Username Key](/glossary/username-key)**, the official identity verification code, especially if your name is common.
...
Important: `wa.me/username` is now WhatsApp's documented link format, but right now it **still doesn't automatically open a conversation for most people**, because the underlying feature is in a phased regional rollout. Whoever receives your link or scans the QR code may need to open WhatsApp and search for your username manually inside the app until the rollout reaches their region. As soon as it's live everywhere, links start working with no changes needed on your end.
